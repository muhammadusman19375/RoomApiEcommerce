...
class ElectronicFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        var view: View = inflater.inflate(R.layout.fragment_electronic, container, false)

        menRcView = view.findViewById(R.id.electronicRcView)
            ...
            override fun onResponse(call: Call<List<ProductsResponse>>, response: Response<List<ProductsResponse>>) {
                if(response.body() != null) {
                    ...
                    for(i in 0..list1.size-1){
                        if(list1[i].category.equals("electronics")){
                            list2.add(list1[i])
                        }
                    }
