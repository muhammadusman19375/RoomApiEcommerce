...
class JewleryFragment : Fragment() {
    var menRcView: RecyclerView? = null
    var progressBar: ProgressBar? = null

    @SuppressLint("MissingInflatedId")
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        var view: View = inflater.inflate(R.layout.fragment_jewlery, container, false)

        menRcView = view.findViewById(R.id.jewleryRcView)
            ...
            override fun onResponse(call: Call<List<ProductsResponse>>, response: Response<List<ProductsResponse>>) {
                if(response.body() != null) {
                    ...
                    for(i in 0..list1.size-1){
                        if(list1[i].category.equals("jewelery")){
                            list2.add(list1[i])
                        }
                    }
